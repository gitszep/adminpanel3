...
export default function ProductForm({
  ...
  async function saveProduct(ev) {
    ev.preventDefault();
    const data = {
      title,description,details,price,images,category,
    ...
    <form onSubmit={saveProduct}>
      ...
      {propertiesToFill.length > 0 && propertiesToFill.map(p => (
        <div key={p.name} className="">
          ...
          <div>
            <select
              multiple={p.multiplePerProduct}
              value={p.multiplePerProduct ? (productProperties[p.name] || '').split(',') : productProperties[p.name]}
              onChange={ev =>
                setProductProp(p.name, [...ev.target.selectedOptions].map(o => o.value).join(','))
              }
            >
              {p.values.map(v => (
