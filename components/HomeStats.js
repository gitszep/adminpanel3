...
export default function HomeStats() {
  ...
  function ordersTotal(orders) {
    ...
    orders.forEach(order => {
      sum+= (order?.tpayTransaction?.amount || 0);
    });
  ...
  return (
    <div>
      ...
      <div className="tiles-grid">
        <div className="tile">
          <h3 className="tile-header">Today</h3>
          <div className="tile-number">{formatPrice(ordersTotal(ordersToday))}</div>
          <div className="tile-desc">{ordersToday.length} orders today</div>
        </div>
        <div className="tile">
          <h3 className="tile-header">This week</h3>
          <div className="tile-number">{formatPrice(ordersTotal(ordersWeek))}</div>
          <div className="tile-desc">{ordersWeek.length} orders this week</div>
        </div>
        <div className="tile">
          <h3 className="tile-header">This month</h3>
          <div className="tile-number">{formatPrice(ordersTotal(ordersMonth))}</div>
          <div className="tile-desc">{ordersMonth.length} orders this month</div>
        </div>
      </div>
    </div>
